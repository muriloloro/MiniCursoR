...
library(readxl)
dados <- read_excel("Dados_experimentais.xlsx", sheet = "DBC_Quan")
...
attach(dados)
plot(data = dados, MCG ~ DOSE)

ResMCG <- dbc(trat = DOSE, bloco = BLOCO, resp = MCG, quali = FALSE, sigT = 0.05)
...
reg1 <- ggplot(dados, aes(x = DOSE, y = MCG)) +
...
### Ponto de máxima eficiencia técnica (MET) - REGRESSÃO QUADRÁTICA
#  y = a + bx + cx^3
#  MET = -b / 2c
...
## PLOTANDO O GRÁFICO DE REGRESSÃO COM A MÉDIA DO TRATAMENTO

DOSEM <- c(0, 25, 50, 75)
MCGM <- c(25.155, 28.04, 32.2475, 33.3125)
dados2 <- data.frame(cbind(DOSEM, MCGM))
dados2  
